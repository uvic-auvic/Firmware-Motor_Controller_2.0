 ...
 *          Max RPM value = 32767. If actual rpm exceeds this, function will return 32767.
 */
extern int16_t motor_get_rpm(motors_t motor_x)
{
	...
	*motorToTIM_DIER[motor_x - 1] &= ~motorToTIM_CCxDE[motor_x - 1];
	...
	uint8_t idx = PWM_IN_ARRAY_LENGTH - (*motorToDMA_NTDR[motor_x - 1]); //Index in array that was most recently updated
	uint8_t stop_idx = (idx + PWM_IN_ARRAY_LENGTH -1) % PWM_IN_ARRAY_LENGTH; //Index that is most out of date
	float frequency = 0;
	int16_t rpm = 0;
